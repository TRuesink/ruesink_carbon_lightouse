...
class TestChallengeTask(unittest.TestCase):
    ...
    def test_get_measure_savings_for_date_range(self):
        ...
        measure_1 = building_1.measures[0]
        measure_1_start_of_savings = measure_1.start
        measure_1_end_of_savings = measure_1.end
        measure_1_savings = measure_1.get_savings_for_date_range(start,end)
        for datapoint in measure_1_savings:
            if datapoint.timestamp > measure_1_start_of_savings and datapoint.timestamp < measure_1_end_of_savings:
                self.assertEqual(datapoint.value, 200)
            else:
                self.assertEqual(datapoint.value, 0)
